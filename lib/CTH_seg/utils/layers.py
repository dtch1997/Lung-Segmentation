...
def deconv3d_wo_bias(layer, stride, class_num, batch_size, mid_size=32, name="deconv2d_wo_bias"):
    ...
    weight_shape = [stride*2, stride*2, stride*2, class_num, c]
    with tf.device("/cpu:0"):
        with tf.variable_scope(name+"_param"):
            W = tf.get_variable("W", weight_shape,
                                initializer=tf.random_normal_initializer(stddev=0.01))
    layer = tf.nn.conv3d_transpose(layer, W, [batch_size, m*stride, n*stride, s*stride, class_num],
                                   strides=[1,stride,stride,stride,1], padding='SAME')
    tf.add_to_collection(layer, 'checkpoints')
    return layer
...
def conv3d_wo_bias(layer, filt_size, filt_num, stride=1, mid_size=0, name="conv2d_wo_bias"):
    ...
    weight_shape = [filt_size[0], filt_size[1], filt_size[2], input_size[4], filt_num]
...
def batch_norm(layer, is_training, beta=0.0, gamma=1.0, decay=0.999, stddev=0.002, name="bn"):
    ...
    INPUTS:
    - layer: (tensor.4d) input of size [batch_size, layer_width, layer_height, channels]
    - is_training: (bool) are we in training mode?
    ...
    # Adding the moving mean and variance to a collection
    tf.add_to_collection(tf.GraphKeys.MOVING_AVERAGE_VARIABLES, mov_mean)
    tf.add_to_collection(tf.GraphKeys.MOVING_AVERAGE_VARIABLES, mov_var)
    ...
    updates = update_mean_var()
    if is_training:
        mean, var = updates # Update the variables in the training network
    else:
...
def bn_relu_conv3d(layer, is_training, filt_size, filt_num, stride=1, mid_size=0, alpha=0.0, name="bn_relu_conv3d"):
    # Normalization
    layer = batch_norm(layer, is_training, name=name)
    # ReLU
    if alpha != 1:
...
def conv3d_bn_relu(layer, is_training, filt_size, filt_num, stride=1, mid_size=0, alpha=0.0, with_batch_norm=True, name="conv3d_bn_relu"):
    """
    A simple 3-dimensional convolution layer.
